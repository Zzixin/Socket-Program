...
int main(int argc, char *argv[]){
    ...
    */
    ...
    if ((rv=getaddrinfo(LocalHost, Central_port_TCPB, &hints, &servinfo)) != 0){
    ...
    //get dynamically assigned portnumber -- from project pdf
    sin_size = sizeof B_addr;
    int Getsock_check = getsockname(sockfd, (struct sockaddr*)&B_addr, &sin_size);
    if (Getsock_check == -1){
        perror("getsockname");
        exit(1);
    }
    ...
    if (argc == 2){
        ...
        strcat(names," ");
        strcat(names,argv[2]);
        numbytes = send(sockfd, names, sizeof names, 0); 
        if (numbytes == -1){
            perror("client: send");
            exit(1);
        }
    ...
    if (last_str!="0" && last_str!="1" && last_str!="2" && last_str!="12"){
        if (len ==2){
            printf("Found no compatibility for %s and %s.\n\n", argv[1], record[0].c_str());
        }
        else{
            printf("Found no compatibility for %s and %s.\n", argv[1], record[0].c_str());
            printf("Found no compatibility for %s and %s.\n\n", argv[2], record[0].c_str());
        }
    }

    else{
        //input1 = Oliver---Rachael---Victor King---Victor 1.06 0.45 Victor Oliver King 12;
        if (last_str == "0"){
